...
public class InputManager : MonoBehaviour {
    ...
    public GameObject GetAimedAtObject(float maxDistance = 15f)
    {
        if (cam == null)
            cam = Camera.main;
        ...
        RaycastHit hit;
        if (Physics.Raycast(pointerPos, Vector3.forward, out hit))
        {
    ...
    public Vector3 GetPointerPositionViewport()
    {
        if (cam == null)
            cam = Camera.main;
        ...
        return cam.ViewportToScreenPoint(new Vector3(pointer.anchorMin.x, pointer.anchorMin.y, cam.nearClipPlane));
    }
    ...
    public void RumbleWiimoteForSeconds(float durationInSeconds)
    {
        ...
        Thread.Sleep((int)(durationInSeconds * 1000)); // Wait for specified number of seconds
                                                       // The Thread.Sleep() function is used instead of Coroutine because it blocks the thread, which means that other code in the same thread won’t be executed until Thread.Sleep() is exited. 
                                                       // This is essential because rumble needs to be turned off only after the vibration has finished.
    ...
    IEnumerator PlayLEDEffectCoroutine(float totalDurationInSeconds)
    {
